...
// convenience function for extracting siphash keys from hashed_header
void setKeysFromHeader(const std::string &hashed_header, siphash_keys *keys);

bool verify(const std::array<edge_t, 42> solution_arr, const std::string& hashed_header, const uint &edge_precentage = EDGE_PRECENTAGE);
