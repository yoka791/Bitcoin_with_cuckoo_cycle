...
void setKeysFromHeader(const std::string &hashed_header, siphash_keys *keys)
{
  setkeys(keys, hashed_header.c_str());
}

bool verify(const std::array<edge_t, 42> solution_arr, const std::string& hashed_header, const uint &edge_precentage)
{
	assert(solution_arr[0] != 0);
	siphash_keys sip_keys;
	setKeysFromHeader(hashed_header, &sip_keys);
	...
	for (size_t i = 0; i < PROOFSIZE; i++)
	{
		if (i != 0) {
			...
			if (j == PROOFSIZE) {
				return even_val_to_search == first_even_index ? true : error("j == PROOFSIZE");
			}
		}
		even_nodes[index] = 0;
