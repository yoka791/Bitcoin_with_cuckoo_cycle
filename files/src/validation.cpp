...
bool ReadBlockFromDisk(CBlock& block, const CDiskBlockPos& pos, const Consensus::Params& consensusParams)
{
	...
	//if (!CheckProofOfWork(SerializeHash(block.cycle_arr), block.nBits, consensusParams) || !verify(block.cycle_arr, block.GetHash().ToString()))	
    //    return error("ReadBlockFromDisk: Errors in block header at %s", pos.ToString());
	//assert(block.cycle_arr[0] == 11 && block.cycle_arr[41] == 2340975);
	//return error("hash of genesis is: %s", block.GetHash().ToString());
...
static bool CheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW = true)
{
    ...
    if (fCheckPOW && (!CheckProofOfWork(SerializeHash(block.cycle_arr), block.nBits, consensusParams) || !verify(block.cycle_arr, block.GetHash().ToString())))
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if (fCheckMerkleRoot) {
		...
		std::string err_str = "hashMerkleRoot mismatch. block merkel: " + block.hashMerkleRoot.ToString() + " computed merkel: " + hashMerkleRoot2.ToString();
        if (block.hashMerkleRoot != hashMerkleRoot2)
            return state.DoS(100, false, REJECT_INVALID, "bad-txnmrklroot", true, err_str);
