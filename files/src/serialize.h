 ...
 * array
 */
template<typename Stream, typename K> void Serialize(Stream& os, const std::array<K, 42>& m);
template<typename Stream, typename K> void Unserialize(Stream& is, std::array<K, 42>& m);
...
void Serialize(Stream& os, const std::array<K, 42>& m)
{
	WriteCompactSize(os, m.size());
...
template<typename Stream, typename K>
void Unserialize(Stream& is, std::array<K, 42>& m)
{
	unsigned int nSize = ReadCompactSize(is);
	for (unsigned int i = 0; i < nSize; i++)
	{
